...
let message_of_bytes bytes =
  { op = Bytes.sub bytes 0 1 |> Op.boot_op_of_bytes |> Option.get
  ; htype = Bytes.sub bytes 1 1 |> GenericInt.t_of_bytes |> GenericInt.int_of_t
  ; hlen = Bytes.sub bytes 2 1 |> GenericInt.t_of_bytes |> GenericInt.int_of_t
  ; hops = Bytes.sub bytes 3 1 |> GenericInt.t_of_bytes |> GenericInt.int_of_t
  ; xid = Bytes.sub bytes 4 4 |> GenericInt.t_of_bytes |> GenericInt.int32_of_t
  ; secs = Bytes.sub bytes 8 2 |> GenericInt.t_of_bytes |> GenericInt.int_of_t
  ; ciaddr = Bytes.sub bytes 12 4 |> Address.addr_of_bytes
  ; yiaddr = Bytes.sub bytes 16 4 |> Address.addr_of_bytes
...
let bytes_of_message message =
  ...
  blit
    (message.sname |> of_string)
    0
    output_buffer
    44
    (min (String.length message.sname) 64);
  blit
    (message.file |> of_string)
    0
    output_buffer
    108
    (min (String.length message.file) 128);
