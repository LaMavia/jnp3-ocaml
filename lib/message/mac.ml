let readable_of_bytes length_bytes bytes =
  ...
  Seq.unfold
    (fun i ->
      if i < length_bytes
      then Option.some (Bytes.get_uint8 addr_bytes i, i + 1)
      else Option.none)
    0
...
let lookup_ip_by_mac ~length_bytes ~mac_address =
  ...
  let stringified_mac_address =
    readable_of_bytes length_bytes mac_address |> Filename.quote
  in
  let command =
    Printf.sprintf "arp -n | grep '%s' | awk '{print $1}'" stringified_mac_address
  in
  try
    (* https://www.ocamlwiki.com/wiki/OCaml_Shell_Command_Execution *)
    let command_output = Unix.open_process_in command in
    let addr = input_line command_output |> Unix.inet_addr_of_string in
    Unix.close_process_in command_output |> ignore;
    Some addr
  with
  | Unix.Unix_error (code, func_name, func_param) ->
    Printf.eprintf
      "Unix error %s with %s(%s)\n"
