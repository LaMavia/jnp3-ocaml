...
let string_of_hash tbl string_of_key string_of_val =
  Hashtbl.to_seq tbl
  |> Seq.fold_left (fun u (k, v) -> u ^ string_of_key k ^ " = " ^ string_of_val v ^ "\n") ""
  |> fun s -> "{" ^ s ^ "}"
...
let db_entry_of_line line = 
  ...
  | f::fs -> 
    ...
    in Printf.eprintf "%s" expl; invalid_arg expl
...
let parse_database_file lines = 
  ...
  with 
    | _ -> Result.ok { 
      homedir = ""; 
      boot_files = Hashtbl.create 0;  
      clients = Hashtbl.create 0;
    }
...
let db_of_path db_file_path = 
  ...
  let rec read_lines lines =
    ...
    in 
  let res =  read_lines [] |> parse_database_file
  in res
